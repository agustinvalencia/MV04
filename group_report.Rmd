...
```{r, echo=FALSE}
mEigenDecom <- eigen(M)
mEigVect <- mEigenDecom$vectors
mEigVals <- mEigenDecom$values
```

Eigenvalues $\overrightarrow{\alpha}$:
```{r, echo=FALSE}
kable(mEigVals)
mEigVals <- mEigVals[1:2]
```

Eigenvectors $\mathbf{\overrightarrow{e}}$:
```{r, echo=FALSE}
kable(mEigVect)
```
...
test2 <- -(n-1-(0.5*(p+q+1))) * log(prod(1-rho2^2))  #13.74948 they are the same
```
