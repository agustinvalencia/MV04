...
A <- sqrtm(sigma11) %*% sigma12 %*% solve(sigma22) %*% sigma21 %*% sqrtm(sigma11)
B <- sqrtm(sigma22) %*% sigma21 %*% solve(sigma11) %*% sigma12 %*% sqrtm(sigma22)
...
$$A = S_{11}^{-1/2}S_{12}S_{22}^{-1}S_{21}S_{11}^{-1/2} = $$
```{r, echo=FALSE}
kable(A)
```
...
```{r, echo=FALSE}
aEigenDecom <- eigen(A)
aEigVect <- aEigenDecom$vectors
aEigVals <- aEigenDecom$values
```

Eigenvalues $\overrightarrow{\alpha}$:
```{r, echo=FALSE}
kable(aEigVals)
```

Eigenvectors $\mathbf{\overrightarrow{a}}$:
```{r, echo=FALSE}
kable(aEigVect)
```
...
$$B = S_{22}^{-1/2}S_{21}S_{11}^{-1}S_{12}S_{22}^{-1/2} = $$
...
```{r, echo=FALSE}
bEigenDecom <- eigen(B)
bEigVect <- bEigenDecom$vectors
bEigVals <- bEigenDecom$values
```
...
Eigenvectors $\mathbf{\overrightarrow{b}}$:
```{r, echo=FALSE}
kable(bEigVect)
```

$$\rho_1 = \sqrt{\overrightarrow{\alpha}}$$
```{r}
ro1 <- sqrt(aEigVals)
kable(ro1)
```

$$\rho_2 = \sqrt{\overrightarrow{\beta}}$$
```{r}
ro2 <- sqrt(bEigVals)
kable(ro2)
```
